...
select region_id, count(node_id) as nodes 
from customer_nodes
group by region_id
order by region_id;
...
select c.region_id, r.region_name, count(distinct(c.customer_id)) as customers
from customer_nodes as c
inner join Regions as r
on  c.region_id = r.region_id
group by c.region_id, r.region_name;

-- Q4. How many days on average are customers reallocated to a different node? TBD with Lareb
...
WHERE end_date!='9999-12-31';
  ...
  -- Q5. What is the median, 80th and 95th percentile for this same reallocation days metric for each region? TBD with Lareb
...
percentile_cte AS (
  SELECT 
    *,
    ((RANK() OVER (PARTITION BY region_id ORDER BY reallocation_days) - 1) 
      / (COUNT(*) OVER (PARTITION BY region_id) - 1)) * 100 AS p
  FROM 
...
select txn_type, count(customer_id) as unique_Customer, sum(txn_amount) as total_amt 
from customer_transactions
group by txn_type;
  
-- Q2. What is the average total historical deposit counts and amounts for all customers?

select (count(customer_id)/ (select count(distinct(customer_id)) from customer_transactions)) as avg_deposit_count,
...
select Customer_id, txn_month, net_transaction_amt, sum(net_transaction_amt) over (partition by Customer_id order by  txn_month)
from passbook
;  
